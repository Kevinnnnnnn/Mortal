...
def _resolve_table_columns(cursor: sqlite3.Cursor, table: str) -> Tuple[str, str]:
    """Return the exact identifier and payload column names for ``table``.

    Some SQLite dumps capitalise column names (``ID``/``Data``) while others use
    lowercase variants (``id``/``data``).  We look up the schema and preserve the
    original casing to keep the SELECT statements valid across both variants.
    """

    cursor.execute(f'PRAGMA table_info("{table}")')
    columns = cursor.fetchall()
    if not columns:
        raise RuntimeError(f"failed to inspect table {table}")

    name_lookup = {col[1].lower(): col[1] for col in columns}
    try:
        return name_lookup["id"], name_lookup["data"]
    except KeyError as exc:  # pragma: no cover - defensive
        missing = ", ".join(sorted(name_lookup))
        raise RuntimeError(f"table {table} is missing required columns: {missing}") from exc


def convert_table(cursor: sqlite3.Cursor, table: str, output_dir: Path) -> TableExportResult:
    id_column, data_column = _resolve_table_columns(cursor, table)

    cursor.execute(f'SELECT "{id_column}", "{data_column}" FROM "{table}"')
